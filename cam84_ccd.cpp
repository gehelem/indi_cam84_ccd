...
void Cam84CCD::grabImage()
{
   ...
   /*for (int i=0; i < height ; i++)
     for (int j=0; j < width; j++)
         image[i*width+j] = (uint8_t) bufim[i][j];*/
...
//unsigned long * img;
  int i,j;
  uint16_t * image2;
  unsigned short *frameBuffer2 = (unsigned short *) image2;
...
// Init
  ...
  while (!cameraGetImageReady) {printf("wait\n");};
  frameBuffer2 = cameraGetImage();
  //for (i = 0; i < 3000; i++)
  //   for (j = 0; j < 2000; j++)
  //    fprintf(stdout,"img %d-%d:%d\n",i,j,bufim[i][j]);
  for (i = 0; i < 300; i++) fprintf(stdout,"img %d:%d\n",i,image2[i]);
  cameraDisconnect();
