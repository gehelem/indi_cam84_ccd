...
void readframe ( int bin,int expoz )
{
    ...
    {
        FT_Out_Buffer[adress+0]=0xD9;       //0xD9 to 0x99 toggles SDA_OUT = AD9826 SDATA
...
bool cameraSetBaudrate ( int val )                /*stdcall; export;*/
{
    ...
    if ( ( val>=minBaudrate ) & ( val<=maxBaudrate ) )
    {
        ...
        spusb = val*1000;
        ms1 = spusb / 8000;  //used to generate a millisecond of delay via output baud rate??????
...
{
    int rsize, rsize_new;
    if(FT_Read ( ftdi, buf, size, &rsize)!=FT_OK)
    ...
    while ( ( nsize>0 ) & ( retry<20 ) )
    {
        ...
        usleep ( 10 );
        //     fprintf ( stderr,"Try %d since %d<>%d \n",retry, rsize,size );
        if(FT_Read ( ftdi, buf, size, &rsize_new)!=FT_OK)
                fprintf(stderr, "FT_READ failed\n");
